...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
        ...
        setStatus('error'); // Можно обработать ошибку
        // Для теста оставим таймаут, если бэкенд еще не настроен идеально
        setTimeout(() => {
             setStatus('success');
             setFormData({ name: '', phone: '', car: '', amount: '' });
        }, 1000);
  ...
  return (
    <main className="min-h-screen font-sans bg-white text-gray-900">
      ...
      {/* --- НАВИГАЦИЯ (Белая, непрозрачная для четкости) --- */}
      ...
      <nav className="fixed top-0 w-full z-50 bg-black/90 backdrop-blur-md border-b border-white/10 shadow-sm transition-all">
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-20 md:h-28 flex justify-between items-center">
          
          {/* 1. Логотип: Адаптивный размер 
              На мобильном: h-12 w-36 (поменьше)
          ...
          */}
          ...
          {/* 3. Кнопка "Оставить заявку"
              На мобильном: Маленькая, желтая иконка трубки или компактная кнопка
              На ПК: Большая желтая кнопка
          */}
          <div className="flex gap-2">
              {/* Версия для мобильных (только трубка) */}
      ...
      {/* --- HERO SECTION (Светлая) --- */}
      <section className="relative min-h-[90vh] flex items-center pt-24 overflow-hidden">
        {/* Фон с фото и БЕЛЫМ градиентом */}
        <div className="absolute inset-0 z-0">
          <Image 
            ...
            priority
          />
          {/* Градиент от белого к прозрачному - чтобы текст был на белом */}
          <div className="absolute inset-0 bg-gradient-to-r from-white via-white/80 to-transparent" />
          {/* Дополнительный слой белого снизу для перехода в следующий блок */}
        ...
        <div className="max-w-7xl mx-auto px-6 z-10 w-full grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            <h1 className="text-5xl md:text-7xl font-extrabold leading-tight mb-6 text-gray-900 drop-shadow-sm">
              Деньги под залог <span className="text-yellow-500">Авто</span> <br/>
              в Дмитрове
            </h1>
            <p className="text-xl text-gray-700 mb-10 max-w-lg font-medium leading-relaxed">
            ...
            <div className="flex gap-6">
              <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
                <p className="text-yellow-500 font-extrabold text-4xl">30 <span className="text-lg text-gray-400 font-bold">мин</span></p>
                <p className="text-sm font-bold uppercase text-gray-600 mt-1">Одобрение</p>
              </div>
              <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
                <p className="text-yellow-500 font-extrabold text-4xl">от 5%</p>
                <p className="text-sm font-bold uppercase text-gray-600 mt-1">Ставка</p>
              </div>
            </div>
          </motion.div>

          {/* Калькулятор: БЕЛЫЙ с тенью */}
          <motion.div 
             id="form"
             initial={{ opacity: 0, y: 50 }}
             animate={{ opacity: 1, y: 0 }}
             className="bg-white p-10 rounded-[2rem] shadow-2xl border border-gray-100 relative z-20"
          >
            <h2 className="text-3xl font-bold mb-8 text-center text-gray-900">Узнайте сумму займа</h2>
            
            <div className="mb-8 space-y-6">
              <div>
                <div className="flex justify-between mb-2 text-base font-bold text-gray-700">
                  <span>Сумма:</span>
                  <span className="text-yellow-600 text-2xl">{calcAmount.toLocaleString()} ₽</span>
                </div>
                <input 
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <input required placeholder="Ваше Имя" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 text-gray-900 placeholder:text-gray-400 focus:bg-white focus:border-yellow-400 focus:ring-4 focus:ring-yellow-100 outline-none transition font-semibold"
                value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
              
              <input required placeholder="Телефон" type="tel" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 text-gray-900 placeholder:text-gray-400 focus:bg-white focus:border-yellow-400 focus:ring-4 focus:ring-yellow-100 outline-none transition font-semibold"
                value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} />
              
              <input required placeholder="Автомобиль (Марка, Модель)" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 text-gray-900 placeholder:text-gray-400 focus:bg-white focus:border-yellow-400 focus:ring-4 focus:ring-yellow-100 outline-none transition font-semibold"
                value={formData.car} onChange={e => setFormData({...formData, car: e.target.value})} />
              
              <button disabled={status === 'loading'} className="w-full bg-black hover:bg-gray-800 text-white font-extrabold py-5 rounded-xl transition text-lg shadow-xl shadow-gray-900/20 mt-2">
      ...
      {/* --- УСЛОВИЯ (Светло-серый фон) --- */}
      <Section id="conditions" className="bg-gray-50">
        <div className="text-center mb-16">
            <h2 className="text-4xl font-extrabold mb-4 text-gray-900">Условия <span className="text-yellow-500">без мелкого шрифта</span></h2>
            <p className="text-gray-600 max-w-2xl mx-auto text-lg font-medium">Максимально прозрачный и быстрый процесс.</p>
        </div>
        
        <div className="grid md:grid-cols-3 gap-8">
          <FeatureCard icon={Clock} title="Деньги за 1 час" desc="От звонка до получения наличных проходит в среднем 60 минут. Мы ценим ваше время." />
          <FeatureCard icon={CheckCircle} title="Любая кредитная история" desc="Нас интересует только залог. Просрочки в банках и отсутствие работы не важны." />
          <FeatureCard icon={FileText} title="Всего 3 документа" desc="Паспорт РФ, ПТС и СТС. Больше никаких справок и поручителей не требуется." />
        </div>
      </Section>
      ...
      {/* --- FAQ (Светло-серый фон) --- */}
      <Section id="faq" className="bg-gray-50">
        <h2 className="text-4xl font-extrabold text-center mb-12 text-gray-900">Частые <span className="text-yellow-500">вопросы</span></h2>
        <div className="max-w-3xl mx-auto space-y-4 bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
          <AccordionItem q="Если у меня плохая кредитная история?" a="Это не имеет значения. Мы выдаем займы под залог имущества, поэтому КИ не проверяется." />
          <AccordionItem q="Как быстро я получу деньги?" a="Обычно вся процедура занимает около часа. Оценка - 15 минут, оформление - 20 минут." />
          <AccordionItem q="Машина остается у меня?" a="Да, у нас есть программа «Займ под ПТС», когда автомобиль остается в вашем пользовании." />
          <AccordionItem q="Какие документы нужны?" a="Паспорт гражданина РФ, ПТС и СТС. Вы должны быть собственником авто." />
        </div>
      </Section>

      {/* --- ФУТЕР (Темный для завершения страницы) --- */}
      <footer id="contacts" className="bg-[#111827] py-20 text-white">
            ...
            <div className="space-y-6 text-gray-300">
               <div className="flex items-start gap-4">
                 <div className="bg-gray-800 p-3 rounded-full">
                    <MapPin className="text-yellow-400 w-6 h-6 flex-shrink-0" />
                 </div>
                 <div>
                    <p className="text-white font-bold mb-1">Адрес офиса:</p>
                    <span className="text-lg">г. Дмитров,<br/>ул. Загорская, д. 22</span>
                 </div>
               </div>
               <div className="flex items-start gap-4 mt-6">
                <div className="bg-gray-800 p-3 rounded-full">
                    <Phone className="text-yellow-400 w-6 h-6 flex-shrink-0" />
                </div>
                <div>
          ...
          <div>
             ...
             <div className="bg-gray-800/50 p-6 rounded-2xl space-y-4 border border-gray-700">
                <div>
                    ...
                    <p className="text-white font-bold text-2xl">10:00 — 22:00</p>
                    <p className="text-yellow-400 font-medium">Ежедневно</p>
                </div>
        ...
        <div className="max-w-7xl mx-auto px-6 mt-16 pt-8 border-t border-gray-800 text-center text-gray-600 text-sm">
            &copy; 2026 DENI-CAR. Не является публичной офертой.
        </div>
      </footer>
    </main>
  );
